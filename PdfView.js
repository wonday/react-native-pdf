...
export default class PdfView extends Component {
    ...
    _getPageWidth = () => {
        ...
        switch (fitPolicy) {
            ...
            default: {
                if (
                    this.state.contentContainerSize.width / this.state.contentContainerSize.height <
    ...
    _renderItem = ({ item, index }) => {
        ...
        if (this.props.singlePage) {
            return <View style={{ flexDirection: this.props.horizontal ? 'row' : 'column' }}>{pageView}</View>;
        }
    ...
    _onViewableItemsChanged = (viewableInfo) => {
        for (let i = 0; i < viewableInfo.viewableItems.length; i++) {
            ...
            if (viewableInfo.viewableItems.length + viewableInfo.viewableItems[0].index < this.state.numberOfPages)
                break;
        }
    };
        ...
        return (
            <PdfViewFlatList
                ...
                contentContainerStyle={[
                    ...
                    this.props.horizontal
                        ? { height: this.state.contentContainerSize.height * this.state.scale }
                ...
                renderScrollComponent={(props) => (
                    <ScrollView {...props} centerContent={this.state.centerContent} pinchGestureEnabled={false} />
                )}
    ...
    render() {
        ...
        return (
            <PinchZoomView style={styles.container} onLayout={this._onLayout} onScaleChanged={this._onScaleChanged}>
                {this.state.pdfLoaded && this._renderList()}
            </PinchZoomView>
        );
    }
